...
pub trait BytesFormat {
    fn serialize_bytes<'a, W: SerWrite, B>(ser: &'a mut Serializer<W, B>, v: &[u8]) -> Result<()>
        where &'a mut Serializer<W, B>: serde::ser::Serializer<Ok=(), Error=SerError>;
}
...
///
/// Serializes bytes as arrays of numbers
...
/// Serialize JSON to a SerWrite implementation
///
/// Serializes bytes as hex strings
...
///
/// Serializes bytes passing them through.
